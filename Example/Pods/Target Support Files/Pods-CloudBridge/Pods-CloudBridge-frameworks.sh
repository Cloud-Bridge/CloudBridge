...
install_framework()
{
  local source="${BUILT_PRODUCTS_DIR}/Pods-CloudBridge/$1"
...
if [[ "$CONFIGURATION" == "Debug" ]]; then
  install_framework 'CBRManagedObjectCache.framework'
  install_framework 'CloudBridge.framework'
  install_framework 'Expecta.framework'
  install_framework 'OCMock.framework'
  install_framework 'SLCoreDataStack.framework'
fi
